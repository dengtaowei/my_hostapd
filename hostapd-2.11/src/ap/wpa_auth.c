...
static const u32 eapol_key_timeout_first = 400; /* ms */
		      ...
		      int keyidx, int encr, int force_version)
{
	...
	if (kde && !encr) {
		...
		if (version == WPA_KEY_INFO_TYPE_HMAC_SHA1_AES ||
		    wpa_use_aes_key_wrap(sm->wpa_key_mgmt) ||
		    version == WPA_KEY_INFO_TYPE_AES_128_CMAC) {
			wpa_hexdump(MSG_DEBUG, "RSN: AES-WRAP using KEK",
					sm->PTK.kek, sm->PTK.kek_len);
	...
	if (key_info & WPA_KEY_INFO_MIC) {
		...
		wpa_hexdump(MSG_DEBUG, "dtw gen mic kck", sm->PTK.kck, sm->PTK.kck_len);
		wpa_hexdump(MSG_DEBUG, "dtw gen mic eapol", hdr, len);
			      ...
			      u8 *data, size_t data_len)
{
	...
	if (wpa_eapol_key_mic(PTK->kck, PTK->kck_len, akmp,
		...
		ret = -1;
	wpa_hexdump(MSG_DEBUG, "dtw ap mic", mic_pos, mic_len);
	wpa_hexdump(MSG_DEBUG, "dtw sta mic", mic, mic_len);
...
SM_STATE(WPA_PTK, AUTHENTICATION2)
{
	...
	unsigned char fix_anonce[] = {0xe5, 0xd1, 0x4c, 0x5e, 0xcf, 0xa8, 0x21, 0xab, 0x61, 0xa7, 0xe2, 0x48, 0xf7, 0xc9, 0x6e, 0x1b, 0x07, 0x03, 0xc6, 0xef, 0xfe, 0x8a, 0x35, 0xeb, 0x2d, 0xce, 0xa9, 0xbb, 0xea, 0x1f, 0x2d, 0xd5};
	memcpy(sm->ANonce, fix_anonce, sizeof(sm->ANonce));
