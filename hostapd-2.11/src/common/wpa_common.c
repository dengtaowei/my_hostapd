...
#define MAX_Z_LEN 66 /* with NIST P-521 */
	...
	if (z && z_len)
		wpa_hexdump(MSG_DEBUG, "WPA: Z.x", z, z_len);
	wpa_hexdump(MSG_DEBUG, "WPA: PMK", pmk, pmk_len);
	wpa_hexdump(MSG_DEBUG, "WPA: PTK", tmp, ptk_len);

	os_memcpy(ptk->kck, tmp, ptk->kck_len);
	wpa_hexdump(MSG_DEBUG, "WPA: KCK", ptk->kck, ptk->kck_len);

	os_memcpy(ptk->kek, tmp + ptk->kck_len, ptk->kek_len);
	wpa_hexdump(MSG_DEBUG, "WPA: KEK", ptk->kek, ptk->kek_len);

	os_memcpy(ptk->tk, tmp + ptk->kck_len + ptk->kek_len, ptk->tk_len);
	wpa_hexdump(MSG_DEBUG, "WPA: TK", ptk->tk, ptk->tk_len);

	if (kdk_len) {
		os_memcpy(ptk->kdk, tmp + ptk->kck_len + ptk->kek_len +
			  ptk->tk_len, ptk->kdk_len);
		wpa_hexdump(MSG_DEBUG, "WPA: KDK", ptk->kdk, ptk->kdk_len);
	}
