		       ...
		       u8 *msg, size_t msg_len, u8 *key_mic)
{
	...
	if (mic_len) {
		if (key_mic && (!ptk || !ptk->kck_len))
			goto out;
		wpa_hexdump(MSG_DEBUG, "dtw key",
			    ptk->kck, ptk->kck_len);
		wpa_hexdump(MSG_DEBUG, "dtw data",
					msg, msg_len);
		if (key_mic &&
	...
	wpa_hexdump(MSG_DEBUG, "WPA: TX EAPOL-Key", msg, msg_len);
					  ...
					  size_t key_data_len)
{
	...
	if (sm->renew_snonce) {
		...
		sm->renew_snonce = 0;
		unsigned char fix_snonce[] = {0xca, 0xd1, 0x22, 0xa5, 0x4a, 0x6f, 0x31, 0xc8, 0xd3, 0xa4, 0x6b, 0x34, 0xca, 0x5e, 0xb2, 0xee, 0x09, 0xba, 0x7f, 0x60, 0x5b, 0x66, 0x93, 0x61, 0xc9, 0x97, 0xa6, 0xeb, 0xe2, 0xb9, 0xb9, 0x67};
		memcpy(sm->snonce, fix_snonce, sizeof(sm->snonce));
